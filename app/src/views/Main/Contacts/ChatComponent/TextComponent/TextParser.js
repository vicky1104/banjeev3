...
export const PATTERNS = {
	 ...
	 */
	url: /^(http:\/\/|https:\/\/|http:\/\/www\.|https:\/\/www\.|www\.)?(www)?(([\w\-]+)?\.[a-z]{0,3})/,
 ...
 * @typedef {DefaultParseShape|import('./TextComponent/lib/TextExtraction').CustomParseShape} ParsedTextAddedProps
...
class TextParser extends React.Component {
	...
	/** @returns {import('./TextComponent/lib/TextExtraction').CustomParseShape[]} */
	getPatterns() {
	...
	getParsedText() {
		...
		return textExtraction.parse().map((props, index) => {
			...
			if (props?.whatIsType === "URL") {
				let y = () => {
					if (/^www/.test(props.children)) {
						return "http://" + props.children;
					} else if (
						!/^http/.test(props.children) &&
						!/^https/.test(props.children) &&
						!/^www/.test(props.children) &&
						/.com$/.test(props?.children)
					) {
						return "http://www." + props.children;
					} else {
					...
					<View style={{ flex: 1 }}>
						<RNUrlPreview
							...
							titleStyle={{
								...
								width: 245,
								fontWeight: "bold",
								color: !this.props?.chatContent?.isSender ? "#fff" : "#000",
							}}
							descriptionStyle={{
								marginLeft: -10,
								width: 245,
