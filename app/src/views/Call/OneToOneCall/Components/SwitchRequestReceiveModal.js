...
export default function VideoCallRequestReceiveModal({ acceptVideoRequest }) {
	...
	const rejectVideoCallRequestFun = () => {
		socket.send(
			JSON.stringify({
				action: "REJECT_VIDEO_CALL_REQUEST",
				data: {
					sender: {
					...
					receiver: {
						firstName: switchCall?.data?.sender?.firstName,
						lastName: switchCall?.data?.sender?.lastName,
					},
					receiverId: switchCall?.data?.senderId,
					callId,
					roomId,
				},
			})
				...
				>
					...
					>
						<Text
							...
							fontWeight={700}
						>
							{`${firstName} ${lastName} request to video call`}
						</Text>
						<View
							style={{
								display: "flex",
						...
						>
							<AppFabButton
								...
								onPress={rejectVideoCallRequestFun}
								icon={
									<MaterialIcons
										size={26}
										color="#FFF"
